...
public class AsignAction extends BaseAction {
	...
	public String createAsign() {
		...
		this.getRequest().setAttribute("hospitalList", hospitalList);
	...
	public String addAsign(Asign asign) {
		String id = this.getRequest().getParameter("id");
		Asign a = this.asignService.getAsignById(id);
		a.setStatus("推荐其他医生");
		this.asignService.updateAsign(a);
		asign.setUsersid(a.getUsersid());
		asign.setStatus("未完成");
	...
	public String getDoctorAsign(String number) {
		...
		PageHelper.getUserPage(asignList, "asign", "getDoctorAsign", 10, number, this.getRequest());
		return "admin/xlistasign";
	}

	@RequestMapping("getTodayAsign.action")
	public String getTodayAsign(String number) {
		String doctorid = (String) this.getSession().getAttribute("adminid");
		Asign asign = new Asign();
		asign.setDoctorid(doctorid);
		asign.setOrderdate(VeDate.getStringDateShort());
		List<Asign> asignList = this.asignService.getAsignByCond(asign);
		PageHelper.getUserPage(asignList, "asign", "getTodayAsign", 10, number, this.getRequest());
		return "admin/xlistasign";
	}
	...
	public String queryAsignByCond(String cond, String name, String number) {
		...
		if (cond != null) {
			...
			if ("hospitalid".equals(cond)) {
				asign.setHospitalid(name);
			}
