...
$("#usersid").blur(
		function() {
			...
			var name = $(this).val();
			if (name == "" || name == null) {
				$("#usersid").after("<span id='usersid_msg' style='color: red'>患者用户不能为空</span>");
			}
	});

$("#hospitalid").blur(
		function() {
			$("#hospitalid_msg").empty();
			var name = $(this).val();
			if (name == "" || name == null) {
				$("#hospitalid").after("<span id='hospitalid_msg' style='color: red'>医院不能为空</span>");
			}
	});
