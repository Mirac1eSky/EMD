...
$(function() {
	$('#deptid').change(function() {
		...
		var str = hospitalid + "," + deptid;
		var url = loc + "ajax/getDoctor.action?str=" + str;
		$.ajax({
			type : "get",
			url : url,
			dataType : "json",
			success : function(json) {
				var did = json.did.replace("[", "").replace("]", "").split(",");
				var dname = json.dname.replace("[", "").replace("]", "").split(",");
				var myOptions = '<option value="">---请选择医生---</option>';
				for(var i=0; i<did.length; i++){
					myOptions += '<option value="' + did[i] + '">' + dname[i] + '</option>';
				}
				$("#doctorid").empty();
